...
import { AddToWishList } from "@shared/components/wishlist-button";
import RemoveItem from "@shared/ecommerce/cart/remove-item";
import QuantityInput from "@shared/ecommerce/cart/quantity-input";
    ...
    <div className="grid grid-cols-12 items-start gap-4 border-b border-muted py-6 first:pt-0 sm:flex sm:gap-6 2xl:py-8">
      ...
      <div className="col-span-8 sm:block sm:w-full">
        <div className="flex flex-col-reverse gap-1 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            <Link to={routes.eCommerce.productDetails(product?.slug as string)}>
              {product.name}
            </Link>
          </Title>
